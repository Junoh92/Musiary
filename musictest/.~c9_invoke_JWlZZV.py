...
@login_required
def new1_search(request):
    return render(request, 'new1_search.html')
    
@login_required
def new2_search(request):
    music1 = request.GET('music1')
    return render(request, 'new2_search.html')
    
@login_required
def new3_search(request):
...
###############################################정보 다 따오자 
    ...
    driver = webdriver.Chrome(chrome_options=chrome_options)
    driver.implicitly_wait(3)
    driver.get(f'https://music.bugs.co.kr/search/integrated?q={singer1} {music2}')
    print('벅스 검색 성공')
##################
    #검색결과 1위 따오기 
    try: 
